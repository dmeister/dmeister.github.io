...
We rely a lot on code reviews for knowledge transfer, but relying on code reviews for knowledge transfer doesn't scale well after a certain point.
...
Knowledge transfer isn't getting easier with our style of C++, which differs in style and substance from general practice.
...
- __Reflect what the consensus is__. A TotW is not a place to publish new ideas. It should always reflect
  what the consensus about what should be done is. Each article is reviewed by other engineers and tech
  leads. They are good to keep you honest. I once wrote an article about "East Const" and it was vetoed
  because while I assumed "Prefer East Const" reflected the consensus at Pure, it actually didn't.
...
What topics did we cover in the last six months?
...
- TotW/15: An article about the principles of physical design and why we use `struct` more often than `class`. This is one of the questions that we get a lot from new hires because it looks very odd, but there are actually good reasons behind our approach to physical design.
...
- TotW/20: Tips on when and when not to use the Curiously Recurring Template Pattern (CRTP)
...
We purposefully switch topics and even the difficulty so that even when one article wasn't interesting to an engineer, there is a good chance that the next one might be.

## Conclusion

The feedback has been very positive. The tips are actively cited in code reviews, which was the main goal. The tips have become an integrated piece of our training and knowledge transfer. One thing I learned is how difficult it is to acquire more authors. Many senior engineers are active with reviewing articles (often multiple times) and ideas. However, getting people to write articles is hard. Of 26 articles, only four have been written by other authors. My goal for the next 26 articles is that I write less than half of them by doubling down on outreach and outright bartering.
...
I wish we could have a technical writer, e.g. from our user documentation team, supporting the project.

## Summary

It is an approach to knowledge transfer with medium effort, focused on the engineer, and provides results. It doesn't require tooling changes or workflow changes. It doesn't require any special infrastructure. A mailing list and a Wiki page would do it.
